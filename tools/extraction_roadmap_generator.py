...
Impact: 30 min manual → 5 min automated!
"""
...
class ExtractionRoadmapGenerator:
    ...
    def generate_roadmap(self, cycles: int = 4) -> str:
        ...
        roadmap = []
        roadmap.append(f"# 🎯 EXTRACTION ROADMAP")
        roadmap.append(f"")
        ...
        for cycle_num in range(1, cycles + 1):
            ...
            roadmap.append(f"")
            roadmap.append(f"## 📅 CYCLE {cycle_num} ({cycle_points} points, ~{cycle_hours:.1f} hours)")
            roadmap.append(f"")
            
            for pattern in patterns_in_cycle:
                emoji = {"CRITICAL": "🚨", "HIGH": "⭐", "MODERATE": "💡", "LOW": "📌"}.get(pattern['priority'], "•")
...
def load_from_analysis_file(filepath: Path) -> List[Dict[str, Any]]:
    ...
    print(f"⚠️  Manual pattern entry recommended for now")
...
def main():
    ...
    # Load from file if provided
        ...
        try:
            ...
            print(f"✅ Loaded {len(generator.patterns)} patterns from {args.input_file}")
        except Exception as e:
            print(f"❌ Failed to load patterns: {e}")
            return
    else:
        print(f"❌ Must provide --from file or use --interactive!")
        parser.print_help()
        return
    
    # Generate roadmap
    if not generator.patterns:
        print(f"❌ No patterns to generate roadmap from!")
        return
    
    print(f"\n🚀 Generating roadmap for {len(generator.patterns)} patterns...")
